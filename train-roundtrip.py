...
def train_one_epoch(G: 'generator model', 
    ...
    for iteration, data in enumerate(dataloader):
        ...
        with torch.no_grad():
            netarc_embeds = netArc(F.interpolate(Xs, [112, 112], mode='bilinear', align_corners=False))
        ...
        with torch.no_grad():
            Y_resized_112 = F.interpolate(Y, [112, 112], mode='bilinear')
            ZY = netArc(Y_resized_112)

        Y_round_trip_to_Xs, _ = G(Xs, ZY)
        round_trip_loss = F.mse_loss(Xs, Y_round_trip_to_Xs)
        ...
        round_trip_loss_multiplier = 1000
        #while universal_multiplier*round_trip_loss_multiplier*round_trip_loss.item() < 500:
        ...
        if iteration % args.show_step == 0:
            images = [Xs, Xt, Y_round_trip_to_Xs, tY, Y]
