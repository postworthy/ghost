...
def train_one_epoch(G: 'generator model', 
    ...
    for iteration, data in enumerate(dataloader):
        ...
        #Added to attempt tp pull colors back into alignment, this may need to be removed if detail is lost
        multiscale_color_loss = multiscale_color_loss_func(Y, tY)
        ...
        #Upsample Loss
        with torch.no_grad():
            Y_upsampled = upscale(Y)

        #Y_upsampled    
        #Xt_eyes, Xt_heatmap_left, Xt_heatmap_right = detect_landmarks(Y_upsampled, model_ft)
        ...
        multiscale_color_loss_multiplier = 10.0

        upsample_loss_multiplier = 30000 #35000 #25000 #10000 #1000 #200
        #while universal_multiplier*upsample_loss_multiplier*upsample_loss.item() < 10:
        #    upsample_loss_multiplier = upsample_loss_multiplier * 1.1

        #round_trip_loss_multiplier = 30
        #while universal_multiplier*round_trip_loss_multiplier*round_trip_loss.item() < 30:
        #    round_trip_loss_multiplier = round_trip_loss_multiplier * 1.1

        teacher_loss_multiplier = 1.0 #30.0
        #while universal_multiplier*teacher_loss_multiplier*teacher_loss.item() < 2000:
        #    teacher_loss_multiplier = teacher_loss_multiplier * 1.1

        netarc_embeds_loss_multiplier = 3.5
        #while universal_multiplier*netarc_embeds_loss_multiplier*netarc_embeds_loss.item() < 2000:
        ...
        if D:
            total_loss = universal_multiplier * (
                ...
                multiscale_color_loss_multiplier * multiscale_color_loss +
                upsample_loss_multiplier * upsample_loss)
        else:
            total_loss = universal_multiplier * (
                teacher_loss_multiplier * teacher_loss + 
                netarc_embeds_loss_multiplier * netarc_embeds_loss + 
        ...
        if iteration % 10 == 0:
            ...
            print(f'multiscale_color_loss:      {universal_multiplier*multiscale_color_loss_multiplier*multiscale_color_loss.item()}        (x{multiscale_color_loss_multiplier})')
            print(f'upsample_loss:              {universal_multiplier*upsample_loss_multiplier*upsample_loss.item()}        (x{upsample_loss_multiplier})')
